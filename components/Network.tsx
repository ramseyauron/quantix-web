...
const networks = [
  {
    ...
    features: ['SPHINCS+ Signatures', 'STARK Proofs', 'PBFT Consensus', 'Block Rewards: 5 QTX'],
    borderColor: '#00d4ff',
  },
  {
    name: 'Testnet',
    ...
    features: ['Full Feature Parity', 'Test QTX Faucet', 'Developer Tools', 'Fast Sync'],
    borderColor: '#7b2fff',
  },
  {
    name: 'Devnet',
          ...
          {networks.map(n => (
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <h3 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#fff' }}>{n.name}</h3>
                <span style={{
              ...
              <p style={{ color: 'rgba(226,232,240,0.55)', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>{n.desc}</p>

              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {n.features.map(f => (
                  <li key={f} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.85rem', color: 'rgba(226,232,240,0.65)' }}>
                    <span style={{ color: n.borderColor, fontSize: '0.7rem' }}>◆</span>
                    {f}
                  </li>
                ))}
              </ul>
